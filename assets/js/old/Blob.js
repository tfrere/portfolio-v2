...
class Blob {
  constructor(canvas, color, numPoints, margin) {
    ...
    this.margin = margin;
    this.color = color;
    this.color = getComputedStyle(this.canvas).getPropertyValue(
      "--subtle-gray-color"
    );
    this.size = 0;
    this.sizeTime = 0;
  ...
  drawCircle(hasToBeStroke) {
    ...
    if (hasToBeStroke) {
      ...
      ctx.fillStyle = getComputedStyle(this.canvas).getPropertyValue("--blue");
      ctx.fill();
    }

    if (this.hasToDrawALine) {
      ctx.beginPath();
      ctx.lineWidth = 1;
      ctx.strokeStyle = "rgba(0,0,0,0.3)";
      ctx.moveTo(
        this.center.x + this.canvas.width / 5.5,
        this.center.y - this.canvas.width / 5.5
      );
      ctx.lineTo(
        this.center.x - this.canvas.width / 5.5,
        this.center.y + this.canvas.width / 5.5
